...
while cycle < total_cycle:
    ...
    if cycle == 0 and idx == 100:
        ...
        cycle += 1
        # if cycle != 0:
        #     up.load_model(fileDir_up,cycle=16)
        #     down.load_model(fileDir_down,cycle=16)
        #     up.change_learning_rate = 0
        #     down.change_learning_rate = 0
    ...
    if idx < len(target_period) / 2 and cycle == 1:
        desire_angle = 30
        up= up_ori

    elif idx >= len(target_period) / 2 and cycle == 1:
        desire_angle = 89
        up.load_model(fileDir_up,cycle=20)


    if idx < len(target_period) / 2 and cycle == 2:
        desire_angle = 89
        up.load_model(fileDir_up,cycle=0)

    elif idx >= len(target_period) / 2 and cycle == 2:
        desire_angle = 30
        up.load_model(fileDir_down,cycle=20)
...
# # FNN 初始化
# up = ANFIS()
# fileDir_up = os.path.join('./exp/','2024_10_21_2042', '1', 'up_model.npy')
...
#         cycle += 1
#         # if cycle != 0:
#         #     up.load_model(fileDir_up,cycle=10)
#         #     up.change_learning_rate(0)
