...
from kerasFuzzy import Torque_ANFIS_multi_pos, go_to_desire_angle
from encoder_function import encoder, moving_average_with_padding,calculate_rmse
import serial
...
# FNN 初始化
up = Torque_ANFIS_multi_pos()
down = Torque_ANFIS_multi_pos()
...
# 主程序函数：用于数据采集与计算控制
def control_loop():
    global h_train,train_torque,train_desire_torque,voltage_his,angle_his, up_model_his, down_model_his,delta_error_his
    ...
    while cycle < 11:
        ...
        if cycle != 0 and idx == len(target_period):
            cycle += 1
            print("reset system, next cycle:",cycle)
        ...
        # # print(desire_torque,f_angle,controller_u)
...
def fnn_train_up():
    ...
    while not stop_event.is_set():  # 检查是否有停止信号
        ...
        # print(local_torque,h_local,local_desire_torque,local_torque)
        # print([error], [h_local], [local_desire_torque], [local_torque])
        up.train([error], [h_local], [local_desire_torque], [local_torque])
...
def fnn_train_down():
    ...
    while not stop_event.is_set():  # 检查是否有停止信号
        ...
        # error = local_desire_torque - local_torque
        # print([error], [h_local], [local_desire_torque], [local_torque])
        down.train([error], [h_local], [local_desire_torque], [local_torque])
